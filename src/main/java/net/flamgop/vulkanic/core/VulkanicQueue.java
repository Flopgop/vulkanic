...
public record VulkanicQueue(int family, @NotNull VkQueue handle) {
    ...
    public int submit(@NotNull VulkanicFence fence, @NotNull List<VulkanicSemaphore> waitSemaphores, @NotNull List<VulkanicSemaphore> signalSemaphores, @NotNull VulkanicCommandBuffer... commandBuffers) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            for (int i = 0; i < waitSemaphores.size(); i++) {
                VulkanicSemaphore semaphore = waitSemaphores.get(i);
            ...
            for (int i = 0; i < signalSemaphores.size(); i++) {
                VulkanicSemaphore semaphore = signalSemaphores.get(i);
